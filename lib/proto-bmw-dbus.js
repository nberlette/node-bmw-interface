...
function clearAll(message, callback = null) {
	status.intf[module_name].errors.resets++;

	log.lib(proto[module_name].inputQueue.length + ' ' + message + ' - resets: ' + status.intf[module_name].errors.resets + ', errors: ' + status.intf[module_name].errors.current);

	status.intf[module_name].errors.current = 0;

	proto[module_name].inputQueue = [];
...
function errorReset(callback = null) {
...
function errorIncrement(message, callback = null) {
...
function check_inputQueue() {
	...
	if (proto[module_name].inputQueue.length >= proto.config.length_max) {
		proto[module_name].parsing = false;
...
async function pusher(data = null) {
	if (data !== null) await proto[module_name].inputQueue.push(data);

	// Start the parser if need be
	if (proto[module_name].parsing === false) await parser();
}

// Emit a data event on each complete data bus message
async function parser() {
	// Bail if the input queue is invalid
	if (!check_inputQueue()) return;
	...
	proto[module_name].inputQueue = await proto[module_name].inputQueue.slice(process_return.slice);
...
async function process() {
	...
	if (msg.len_full < proto[module_name].length_min) {
		errorIncrement('Message too short (mLenT ' + msg.len_full + ' < cMin ' + proto[module_name].length_min + ')');
	...
	if (msg.len_full > proto.config.length_max) {
		errorIncrement('Message too long (mLenT ' + msg.len_full + ' > cMax ' + proto.config.length_max + ')');
	...
	if (!validateModule('dst', msg.dst.id)) {
		errorIncrement('Invalid destination');
	...
	if (msg.len_full > queue_process.length) {
		// errorIncrement('Not enough data (mLenT '+msg.len_full+' > bLen '+queue_process.length+')');
		return {
...
async function create(msg) {
	...
	buffer[(msg.msg.length + proto[module_name].crc_offset)] = await calculate_crc(buffer);
