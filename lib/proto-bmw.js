...
async function processInputQueue() {
	...
	if (inputQueueClone.length < proto.proto.msgLengthMin) {
		errorIncrement(`Input queue too short (${inputQueueClone.length})`);
	...
	if (msg.lenFull < proto.proto.msgLengthMin) {
		errorIncrement('Message too short (mLenT ' + msg.lenFull + ' < cMin ' + proto.proto.msgLengthMin + ')');
	...
	if (msg.lenFull > proto.config.msgLengthMax) {
		errorIncrement('Message too long (mLenT ' + msg.lenFull + ' > cMax ' + proto.config.msgLengthMax + ')');
		// console.dir({ msg, inputQueueClone }, { depth : null });
